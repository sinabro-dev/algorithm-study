...
func (c *LRUCache) Put(key int, value int) {
	...
	if ok {
		...
		c.moveToHead(newElement)
		c.table[key] = newElement
		c.len++
		if c.len > c.capacity {
			delete(c.table, c.tail.prev.record.key)
			c.len--
			...
			tmp.prev, tmp.next, tmp = nil, nil, nil
		}
	}
}

func (c *LRUCache) moveToHead(element *Element) {
	if (element.prev != nil) && (element.next != nil) {
		element.prev.next = element.next
		element.next.prev = element.prev
	}
	element.prev = c.head
	element.next = c.head.next

	c.head.next.prev = element
	c.head.next = element
}
...
func convert(s string) (int, []int, [][]int) {
	...
	dataList := make([][]int, 0)
	splitData := strings.Split(inputData, ",")
	row, col := 0, 0
	for _, s := range splitData {
		for _, c := range s {
			if c == '[' {
				dataList = append(dataList, make([]int, 2))
				col = 0
			} else if c == ']' {
				row++
			} else {
				dataList[row][col] = dataList[row][col] * 10 + int(c - '0')
			}
		}
		col++
	}

	return dataList[0][0], commandList, dataList[1:]
}
