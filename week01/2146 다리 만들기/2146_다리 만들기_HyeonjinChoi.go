...
func main() {
	...
	for i := 0; i < sideLen; i++ {
		for j := 0; j < sideLen; j++ {
			if island[i][j] == 1 {
				separateIsland(j, i, sideLen, islandNum, island)
				islandNum++
			}
		}
	}
	...
	for i := 0; i < sideLen; i++ {
		for j := 0; j < sideLen; j++ {
			if island[i][j] > 1 {
				signal = findIsland(j, i, sideLen, island[i][j], island)
				// signal 값이 -1이면 비정상으로 간주
				if signal == -1 {
					continue
				}

				min = compareNum(min, signal)
			}
		}
	}
...
func compareNum(num1, num2 int) int {
	/*
		두 수의 대소 비교
	*/
	if num1 < num2 {
...
func separateIsland(x, y, sideLen, islandNum int, island [][]int) {
	/*
		떨어져있는 섬들을 구분하기 위해서
		bfs를 이용하여
		각각 개별번호 부여하는 함수
...
func findIsland(x, y, sideLen, islandNum int, island [][]int) int {
	/*
		섬의 한 지점에서 다른 섬까지
		최소 거리를 구하는 함수
	*/
